...
class DocSearchApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # Wheel handling is finicky on macOS (events sometimes get delivered
        # to whichever child widget is under the pointer). We bind globally and
        # guard by pointer position so scrolling works anywhere in the results area.
    ...
    def _on_mousewheel(self, event):
        # If wheel events reach here, this label will change immediately.
        ...
        # Only scroll if mouse is inside results area
        if not inside:
            return
        ...
        if hasattr(event, "delta") and getattr(event, "delta", 0) != 0:
            ...
            # Linux fallback: 4 = up, 5 = down
            raw = -120 if event.num == 4 else 120
        else:
            return

        # --- Accumulate small deltas (KEY FIX) ---
        self._wheel_accum += raw

        # Threshold: how much movement = 1 scroll unit.
        # Trackpad deltas can be much smaller than a mouse wheel, so keep this low.
        threshold = 30  # tweakable
        move_units = int(self._wheel_accum / threshold)

        # If deltas are extremely small (common with trackpads), wait-for-threshold
        # can feel like "nothing is happening". Once accumulated movement reaches
        # magnitude >= 1, scroll by 1 unit in the correct direction.
        ...
        if move_units != 0:
            ...
            # Helpful debug when wheel delta is too small to pass the threshold.
            # (do nothing; label already shows accum/move_units state)
            pass
    ...
    def _smooth_scroll_to_frac(self, target_frac, duration_ms=180):
        ...
        # Cancel any previous smooth scroll animation.
    ...
    def _scroll_by_units(self, units):
        """Scroll results smoothly by a number of row steps."""
        if not units:
            return

        delta_px = float(units) * ROW_HEIGHT
        # Duration scales with distance, capped for responsiveness.
    ...
    def _on_scroll_key(self, event, direction):
        # Only scroll when pointer is over results list, to avoid breaking text input.
        if not self._pointer_in_results():
            return

        # direction: -1 for up, +1 for down
        # Cancel wheel inertia so key scroll feels deterministic.
        self._scroll_animating = False
        self._scroll_velocity = 0

        # Use a small step for arrows, larger for page keys.
    ...
    def _ensure_slot_cards(self):
        ...
        while len(self._slot_cards) < N_VISIBLE_SLOTS:
            ...
            # Wheel bindings must be on the child widgets too; otherwise
            # hovering the labels/frames can prevent scrolling.
            for w in (card, type_lbl, main_lbl, side_lbl, doc_lbl, copy_btn):
                w.bind('<MouseWheel>', self._on_mousewheel)
                w.bind('<Button-4>', self._on_mousewheel)  # Linux-style
                w.bind('<Button-5>', self._on_mousewheel)  # Linux-style
    ...
    def _update_slot_content(self, slot, sec, idx):
        ...
        slot['side_lbl'].config(fg='#ffffff')
