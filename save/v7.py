...
def open_file_at_heading_via_word(doc_path, para_idx):
    abs_path = os.path.abspath(doc_path)
    escaped_path = abs_path.replace('\\', '\\\\').replace('"', '\\"')
    para_idx = int(para_idx)
...
class DocSearchApp(tk.Tk):
    def __init__(self):
        ...
        self._source_filter_map = {'All docs': ('all', None)}
        self._doc_filter_var = tk.StringVar(value='All docs')
        ...
        self._build_ui()
        self._wheel_accum = 0  # retained for debug visibility only
        ...
        self._key_scroll_after_id = None
        self._active_scroll_key = None

    def _build_ui(self):
        BG = '#0e0e0e'; SURF = '#161616'; ACC = '#048a2e'; DIM = '#777'; TEXT = '#e2e2e2'
        ...
        # Keyboard scroll with hold-to-repeat (avoids OS key-repeat pause).
        self.bind_all('<KeyPress-Up>', self._on_scroll_key_press)
        self.bind_all('<KeyPress-Down>', self._on_scroll_key_press)
        self.bind_all('<KeyPress-Prior>', self._on_scroll_key_press)    # PageUp
        self.bind_all('<KeyPress-Next>', self._on_scroll_key_press)     # PageDown
        self.bind_all('<KeyRelease-Up>', self._on_scroll_key_release)
        self.bind_all('<KeyRelease-Down>', self._on_scroll_key_release)
        self.bind_all('<KeyRelease-Prior>', self._on_scroll_key_release)
        self.bind_all('<KeyRelease-Next>', self._on_scroll_key_release)
        ...
        source_frame = tk.Frame(toolbar, bg=BG)
        source_frame.pack(side='left', padx=(6, 0))
        tk.Label(source_frame, text='SOURCE:', font=('Courier', 9), fg=DIM, bg=BG).pack(side='left', padx=(0,6))
        self.source_menu = tk.OptionMenu(source_frame, self._doc_filter_var, 'All docs', command=lambda *_: self._refresh_results())
        self.source_menu.config(
            font=('Courier', 9),
            fg=TEXT,
            bg=SURF,
        ...
        self.source_menu['menu'].config(font=('Courier', 9), fg=TEXT, bg=SURF, activeforeground=TEXT, activebackground='#222')
        self.source_menu.pack(side='left')
        self._refresh_doc_filter_menu()
    ...
    def _on_mousewheel(self, event):
        ...
        # Convert wheel delta to pixels and smooth toward a moving target.
        # This avoids unit quantization jitter from yview_scroll(..., "units").
        delta_px = (float(raw) / 120.0) * (ROW_HEIGHT * 0.70)
        max_step = float(ROW_HEIGHT * 4)
        delta_px = max(-max_step, min(max_step, delta_px))
        self._smooth_scroll_by_pixels(delta_px)
    ...
    def _smooth_scroll_to_frac(self, target_frac, duration_ms=180):
        ...
        target_px = target_frac * float(scrollable)
        current_px = self.canvas.yview()[0] * float(scrollable)
        self._smooth_scroll_by_pixels(target_px - current_px, duration_ms=duration_ms)
    ...
    def _smooth_scroll_by_pixels(self, delta_px, duration_ms=180):
        ...
        if self._smooth_scroll_target_px is None:
            self._smooth_scroll_target_px = current_px
        target_px = self._smooth_scroll_target_px + float(delta_px)
        self._smooth_scroll_target_px = max(0.0, min(float(scrollable), target_px))
        if self._smooth_scroll_after_id is None:
            # Start on next frame so wheel input never causes an immediate snap.
            self._smooth_scroll_after_id = self.after(12, self._smooth_scroll_step)

    def _smooth_scroll_step(self):
        scrollable = self._get_scrollable_range_px()
        if scrollable is None or self._smooth_scroll_target_px is None:
            self._smooth_scroll_after_id = None
            return

        current_px = self.canvas.yview()[0] * float(scrollable)
        dist = self._smooth_scroll_target_px - current_px
        if abs(dist) <= 0.5:
            final_frac = self._smooth_scroll_target_px / float(scrollable)
            self.canvas.yview_moveto(final_frac)
            self._update_visible_cards()
            self._smooth_scroll_target_px = None
            self._smooth_scroll_after_id = None
            return

        # Exponential smoothing with a frame cap to avoid first-frame snapping.
        step = dist * 0.28
        max_step = ROW_HEIGHT * 0.45
        if step > max_step:
            step = max_step
        elif step < -max_step:
            step = -max_step
        min_step = 0.6 if dist > 0 else -0.6
        if abs(step) < abs(min_step):
            step = min_step
        next_px = max(0.0, min(float(scrollable), current_px + step))
        self.canvas.yview_moveto(next_px / float(scrollable))
        self._update_visible_cards()
        self._smooth_scroll_after_id = self.after(12, self._smooth_scroll_step)
    ...
    def _cancel_key_scroll(self):
        if self._key_scroll_after_id is not None:
            try:
                self.after_cancel(self._key_scroll_after_id)
            except Exception:
                pass
        self._key_scroll_after_id = None
        self._active_scroll_key = None

    def _on_scroll_key_press(self, event):
        if not self._pointer_in_results():
            return
        keysym = getattr(event, 'keysym', '')
        if keysym not in ('Up', 'Down', 'Prior', 'Next'):
            return
        if self._active_scroll_key == keysym and self._key_scroll_after_id is not None:
            return 'break'
        self._cancel_key_scroll()
        self._active_scroll_key = keysym
        self._on_scroll_key(event, -1 if keysym in ('Up', 'Prior') else 1)
        self._key_scroll_after_id = self.after(42, self._key_scroll_tick)
        return 'break'

    def _on_scroll_key_release(self, event):
        keysym = getattr(event, 'keysym', '')
        if keysym and keysym == self._active_scroll_key:
            self._cancel_key_scroll()
            return 'break'

    def _key_scroll_tick(self):
        keysym = self._active_scroll_key
        if keysym is None or not self._pointer_in_results():
            self._cancel_key_scroll()
            return
        direction = -1 if keysym in ('Up', 'Prior') else 1
        evt = type('E', (), {'keysym': keysym})()
        self._on_scroll_key(evt, direction)
        self._key_scroll_after_id = self.after(34, self._key_scroll_tick)
    ...
    def _doc_group_priority(self, path):
        """
        Priority buckets for display ordering:
          0: topic specific/Aff and topic specific/Neg
          1: general
          2: everything else
        """
        ...
        for i, part in enumerate(parts):
            part_l = part.lower()
            if part_l in topic_labels:
                next_part = parts[i + 1].lower() if i + 1 < len(parts) else ''
                if next_part in {'aff', 'neg'}:
                    return 0
                return 2
            if part_l == 'general':
                return 1
    ...
    def _sort_docs_by_debate_group(self):
        def doc_priority(doc):
            ...
            return (self._doc_group_priority(abs_p), abs_p.lower())
    ...
    def _refresh_doc_filter_menu(self):
        labels = ['All docs']
        source_map = {'All docs': ('all', None)}
        folder_labels = {}
        for doc in self.docs:
            doc_path = os.path.abspath(doc.get('path') or '')
            label = self._doc_display_name(doc_path)
            base_label = f'File: {label or os.path.basename(doc_path) or "Untitled"}'
            unique_label = base_label
            suffix = 2
            while unique_label in source_map:
                unique_label = f'{base_label} ({suffix})'
                suffix += 1
            labels.append(unique_label)
            source_map[unique_label] = ('doc', doc_path)

            dir_path = os.path.dirname(doc_path)
            dir_display = os.path.dirname(label) if label else os.path.basename(dir_path)
            if not dir_display:
                dir_display = dir_path
            folder_label = f'Folder: {dir_display}'
            folder_labels[folder_label] = dir_path

        for folder_label in sorted(folder_labels.keys(), key=str.lower):
            labels.append(folder_label)
            source_map[folder_label] = ('folder', folder_labels[folder_label])

        self._source_filter_map = source_map
        ...
        for label in labels:
            menu.add_command(
                label=label,
                command=lambda v=label: (self._doc_filter_var.set(v), self._refresh_results())
            )

        current = self._doc_filter_var.get()
        if current not in source_map:
            self._doc_filter_var.set('All docs')
    ...
    def _refresh_results(self):
        ...
        selected_label = self._doc_filter_var.get()
        selected_kind, selected_path = self._source_filter_map.get(selected_label, ('all', None))

        type_filtered = []
        for sec in self._all_sections_cache:
            if sec['type'] not in active:
                continue
            sec_doc_path = os.path.abspath(sec.get('docPath') or '')
            if selected_kind == 'doc':
                if sec_doc_path != os.path.abspath(selected_path or ''):
                    continue
            elif selected_kind == 'folder':
                try:
                    if os.path.commonpath([sec_doc_path, os.path.abspath(selected_path or '')]) != os.path.abspath(selected_path or ''):
                        continue
                except Exception:
                    continue
            type_filtered.append(sec)
        if query:
    ...
    def _ensure_slot_cards(self):
        ...
        while len(self._slot_cards) < N_VISIBLE_SLOTS:
            ...
            open_btn = tk.Button(card, text='o', font=('Courier', 9, 'bold'),
                                 fg='#000', bg='#222', activeforeground='#000',
                                 activebackground='#2a2a2a', bd=0, padx=4, pady=2, cursor='hand2')
            open_btn.pack(side='right', padx=2)
            send_btn = tk.Button(card, text='s', font=('Courier', 9, 'bold'),
                                 fg='#000', bg='#222', activeforeground='#000',
                                 activebackground='#2a2a2a', bd=0, padx=4, pady=2, cursor='hand2')
            send_btn.pack(side='right', padx=2)
            copy_btn = tk.Button(card, text='c', font=('Courier', 9, 'bold'),
                                 fg='#000', bg='#222', activeforeground='#000',
                                 activebackground='#2a2a2a', bd=0, padx=4, pady=2, cursor='hand2')
            copy_btn.pack(side='right', padx=2)
            for w in (card, type_lbl, main_lbl, side_lbl, doc_lbl, open_btn, send_btn, copy_btn):
            ...
            self._slot_cards.append({
                ...
                'doc_lbl': doc_lbl, 'open_btn': open_btn, 'send_btn': send_btn, 'copy_btn': copy_btn
            })
    ...
    def _update_slot_content(self, slot, sec, idx):
        ...
        slot['open_btn'].config(bg=border, activebackground=shade, fg='#000', activeforeground='#000')
        ...
        slot['open_btn'].config(command=lambda s=sec, b=slot['open_btn']: self._open_section(s, b))
    ...
    def _render_card(self, sec, idx):
        ...
        copy_btn = tk.Button(hdr_row, text='c', font=('Courier', 9, 'bold'),
                             fg='#888', bg='#222', activeforeground=color,
                             activebackground='#2a2a2a', bd=0, padx=4, pady=2, cursor='hand2')
        copy_btn.pack(side='right', padx=2)
        copy_btn.config(command=lambda s=sec, b=copy_btn: self._copy_section(s, b))
        send_btn = tk.Button(hdr_row, text='s', font=('Courier', 9, 'bold'),
                             fg='#888', bg='#222', activeforeground=color,
                             activebackground='#2a2a2a', bd=0, padx=4, pady=2, cursor='hand2')
        send_btn.pack(side='right', padx=2)
        send_btn.config(command=lambda s=sec, b=send_btn: self._send_section(s, b))
        open_btn = tk.Button(hdr_row, text='o', font=('Courier', 9, 'bold'),
                             fg='#888', bg='#222', activeforeground=color,
                             activebackground='#2a2a2a', bd=0, padx=4, pady=2, cursor='hand2')
        open_btn.pack(side='right', padx=2)
        open_btn.config(command=lambda s=sec, b=open_btn: self._open_section(s, b))
        ...
        def do_copy():
            ...
            def on_done():
                if err:
                    ...
                    self.after(2000, lambda: btn.config(text='c', fg='#888'))
        ...
        def do_send():
            ...
            def on_done():
                if err:
                    ...
                    self.after(2000, lambda: btn.config(text='s', fg='#888'))

            self.after(0, on_done)

        threading.Thread(target=do_send, daemon=True).start()

    def _open_section(self, sec, btn):
        btn.config(text='…', fg='#aaa')
        self.update_idletasks()
        ...
        def do_open():
            err = None
            try:
                open_file_at_heading_via_word(sec['docPath'], sec['para_idx'])
            except Exception as e:
                err = e

            def on_done():
                if err:
                    btn.config(text='o', fg='#888')
                    messagebox.showerror('Open failed',
                        f'Could not open file at heading via Word.\n\nMake sure Microsoft Word is installed.\n\nError: {err}')
                else:
                    btn.config(text='✓ opened', fg='#47ffb8')
                    self.after(2000, lambda: btn.config(text='o', fg='#888'))
